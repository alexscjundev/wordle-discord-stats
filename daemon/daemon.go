...
func New(session *discordgo.Session, channelID, botUserID, imgparseBin string, cursor MessageCursor, st store.Store, nicks *nickcache.NickCache) *Daemon {
	return &Daemon{
		session:     session,
		channelID:   channelID,
		botUserID:   botUserID,
...
func (d *Daemon) postReport() {
	entries, err := d.store.QueryTop(5, 0, store.ScoringAverage)
	if err != nil {
		slog.Error("postReport: query", "err", err)
		return
	}
	msg := buildHeader(time.Now()) + "**Top 5 (all time)**\n" + store.FormatTop(entries, d.nicks.Get)
		...
		for pr := range ch {
			...
			for _, result := range pr.results {
				...
				if err != nil {
					...
					savedNew = true
					slog.Info("daemon: saved", "day", result.Day, "user", result.UserID, "score", result.Score)
				}
			}
		}
